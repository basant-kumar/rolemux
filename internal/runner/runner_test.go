...
func TestNativeWorkerSchemaRequiresEveryProperty(t *testing.T) {
	...
	for _, required := range []string{`"required":["role","status","plan","question","complexity","work_units"]`, `"complexity":{"type":"string","enum":["trivial","small","medium","large","system"]}`, `"required":["id","objective","scope","depends_on","context_group","context_files","affected_symbols","estimated_minutes","execution_packet","acceptance_criteria","validation_commands"]`, `"additionalProperties":false`, `"enum":["planner"]`} {
