...
const (
	implementerRoleDiscipline  = "Implementer execution discipline: before the first edit, use at most six underlying read/search operations across at most two provider tool turns, with at most 16 KiB aggregate discovery output; nested calls inside a batch count separately. Prefer named files and symbols from the execution packet; do not run git status/diff/log, repository-wide searches, repository-wide surveys, or a post-green survey. Consolidate related source and test edits into at most two cohesive edit calls before validation, then use one correction batch when practical. Run focused validation in one batched call; never run the full repository suite. If host-only evidence is required, return one needs_input question without attempting the blocked command repeatedly. Wait at least 30 seconds for a background test or build instead of one-second polling, and stop immediately when focused validation passes."
